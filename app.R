...
# ------------------------------RIVERS--------------------------------------------------------
#---------------------------------------------------------------------------------------------
#To read the CSV input files for all parameters except DO
data <- read.csv('Rivers.csv',fileEncoding="UTF-8-BOM")
#---------------------------------------------------------------------------------------------
#To read the CSV input files for DO
...
# To create empty dataframes for each parameter to be used later on
...
df_summary_tur<-data.frame()
df_summary_tem<-data.frame()
# ---------------------------------------------------------------------------------------------
...
#-----------------------------------------------------------------------------------------------
#Function to get sample count,censored%,median,max,min,exceed 540 and exceed 260 per year
...
#-----------------------------------------------------------------------------------------
#Function to get sample count,censored%,median,max,min,exceed 540 and exceed 260 values for all years
...
for (param in df_param){
  if(param!="pH")
  {
    ...
    if(param=="Ammonia (total)"){
      # For Ammonia and PH, we are combining the dataframes
      ...
      # Equation to get the ratio to divide the value
    ...
    # Looping at the watersheds
    for (ws in df_wshed) {
      data_ws<-filter(data_input,Watershed == ws) #To get watershed specific data
      # For Tamaki we have a separate grading criteria
      if( ws == "Tamaki"){
        grade <- read.csv('Grades_Tamaki.csv',fileEncoding="UTF-8-BOM")
      }else if(ws == "Wairoa"){
        # For Wairoa we have a separate grading criteria
        grade <- read.csv('Grades_Wairoa.csv',fileEncoding="UTF-8-BOM")
      }else{
        # For rest of the watersheds we are using a common grading criteria
      ...
      if(param == "DO"){
        ...
        # -----------------------------------------------------------------------------------------------
        # To combine yearly and all years data
      ...
      for(func in df_function){
        ...
        if(func == "Median")
        {
          # To get the index of the grade interval where the Median Value falls
          ...
          # Based on the index value, we get the grade assigned
          df_summary$Median_Attr_state_yearly <- Gr_func[df_summary$Index,]$Grade
          # Index calculation for All years data
        ...
        {
          # To get the index of the grade interval where the Percentile Value falls
        ...
        {
          # To get the index of the grade interval where the Min Value falls
        ...
        # ------------------------------------------------------------------------------------------------------------
        # To get the Mean attribute state yearly and all years
        else if(func == "Mean")
        {
          # To get the index of the grade interval where the Mean Value falls
        ...
        {
          # To get the index of the grade interval where the Max Value falls
        ...
        {
          # To get the index of the grade interval where the exceedance value falls
      ...
      # To combine the data in a loop with previous data
      if(param == "Ammonia (total)"){
        ...
        df_summary_tur <- rbind(df_summary_tur, df_summary)
      }else if ( param == "Temperature"){
        df_summary_tem <- rbind(df_summary_tem, df_summary)
      }
    }
    # --------------------------------------------------------------------------------------------------------
    # To place parameter-wise data into the final dataframe
    library(tidyr)
    if(param == "Ammonia (total)"){
      ...
      df_summary<-df_summary_tur
    }else if ( param == "Temperature"){
      df_summary<-df_summary_tem
    }
    # df_summary<- df_summary_ws
    df_summary<-df_summary %>%
      separate(Site, c("Site", "SiteId"), "_")
    # --------------------------------------------------------------------------------------------------------
    # To get the summary row showing all years data
    df_sum<-df_summary[!duplicated(df_summary[c(1,2)]),]
    df_sum$Samples_yearly<-df_sum$Samples_All_Years
    df_sum$Year<-"All"
    # Assigning the required calculations and grades parameter-wise
    if(param == "Ammonia (total)"|| param == "Temperature"){
      ...
      df_summary$Med_val<-ifelse(df_summary$Median_Attr_state_yearly=="A",1,
                                 ifelse(df_summary$Median_Attr_state_yearly=="B",2,
                                        ifelse(df_summary$Median_Attr_state_yearly=="C",3,
                                               ifelse(df_summary$Median_Attr_state_yearly=="D",4,
                                                      ifelse(df_summary$Median_Attr_state_yearly=="E",5,"NA")))))
      df_max<-data.frame(df_summary$Med_val)
    }else if(param=="E.coli"){
      # To assign a numeric value to the grade for easier comparison
    ...
    # To get the maximum value from the comparsion
    df_summary$Max_val<-apply(X=df_max, MARGIN=1, FUN=max)
    # To convert the numeric value to corresponding grade
    ...
    if (param=="Total Oxidized Nitrogen"){
      ...
      df_tem_r<-select(df_summary, Watershed, Site, SiteId, Samples_yearly, Censored,
                       Median_yearly, Max_Yearly,
                       Median_Attr_state_yearly, Max_Attr_state_yearly,
                       Overall_Attribute_State)
    }else if(param=="Turbidity"){
      df_tur_r<-select(df_summary, Watershed, Site, SiteId, Samples_yearly, Censored,
                       Median_yearly,
...
#To write parameters into separate tabs of an excel file as output
...
#------------------------------------------------------------------------------------------
# To identify deep(replaced as 1) and shallow(replaced as 0) lakes
for (lake in df_lakes) {
  if(lake == "Tomarata")
  {
    # If the depth of the lake is lesser than or equal to 1 assign 0 else 1
  ...
  {
    # If the depth of the lake is lesser than or equal to 15 assign 0 else 1
  ...
  {
    # If the depth of the lake is lesser than or equal to 4 assign 0 else 1
  ...
  {
    # If the depth of the lake is lesser than or equal to 6 assign 0 else 1
...
#-------------------------------------------------------------------------------------------
#Function to get Yearly data (sample count, median, max, exceed 540 and exceed 260)
...
#-----------------------------------------------------------------------------------------
#Function to get All Years data (sample count, median, max, exceed 540 and exceed 260)
...
for (param in df_param){
  if(param!="pH")
  {
    ...
    if(param=="Ammonia (total)"){
      ...
      data_input1<-merge(data_ip, data_ph, by=c("SampleNumber","Site"))
      df_uph<-data_input1[!duplicated(data_input1[1:5]),]
      # Equation to calculate the ratio
      df_uph$ratio<- ifelse(df_uph$Value.y>9,0.2, -0.029809630224*df_uph$Value.y^6 + 1.2641450933*df_uph$Value.y^5 - 22.08947826*df_uph$Value.y^4 + 203.57964573*df_uph$Value.y^3 - 1043.9962833*df_uph$Value.y^2 + 2825.5522036*df_uph$Value.y - 3151.0179751)
      df_uph$Concph8<-df_uph$Value.x/df_uph$ratio
    ...
    # We consider max of all values for E.Coli and Average for other parameters
    if(param == "E.coli"){
    ...
    for(func in df_function){
      ...
      if(func == "Median")
      {
        # To get the index of the grade interval where the value falls
        findIndex <- function(Median_Yearly) 
        {
          interval.gr[,which((interval.gr$gt) < Median_Yearly & Median_Yearly <= (interval.gr$lt))]
        }
    ...
    if((param != "Total Nitrogen") & (param != "Total Phosphorus (P)")){
      ...
      df_summary$Max_val<-apply(X=df_max, MARGIN=1, FUN=max)
      # To convert the numeric value to respective grade 
      df_summary$Overall_Attribute_State<-ifelse(df_summary$Max_val==1,"A",
                                                 ifelse(df_summary$Max_val==2,"B",
                                                        ifelse(df_summary$Max_val==3,"C",
                                                               ifelse(df_summary$Max_val==4,"D",
...
# To write the parameters into separate tabs of an excel file as output
...
spiderr_tur<-data.frame("Watershed"=df_tur_r$Watershed, "Site"=df_tur_r$Site,
                               "Turbidity"=df_tur_r$Overall_Attribute_State)
spiderr_tem<-data.frame("Watershed"=df_tem_r$Watershed, "Site"=df_tem_r$Site,
                        "Temperature"=df_tem_r$Overall_Attribute_State)
spiderr<-merge(spiderr_ecoli, spiderr_Cu_soluble, by=c('Watershed','Site'), all.x = TRUE)
...
spiderr<-merge(spiderr, spiderr_Zn_soluble, by=c('Watershed','Site'), all.x = TRUE)
spiderr<-merge(spiderr, spiderr_tur, by=c('Watershed','Site'), all.x = TRUE)
spiderr<-merge(spiderr, spiderr_tem, by=c('Watershed','Site'), all.x = TRUE)
                                  ...
                                  ifelse(spiderr$Zn_soluble=="C",3,
                                         ifelse(spiderr$Zn_soluble=="D",4,
                                                ifelse(spiderr$Zn_soluble=="E",5,
                                                       "NA")))))
spiderr$Turbidity<-ifelse(spiderr$Turbidity=="A",1,
                           ifelse(spiderr$Turbidity=="B",2,
                                  ifelse(spiderr$Turbidity=="C",3,
                                         ifelse(spiderr$Turbidity=="D",4,
                                                ifelse(spiderr$Turbidity=="E",5,
                                                       "NA")))))
spiderr$Temperature<-ifelse(spiderr$Temperature=="A",1,
                          ifelse(spiderr$Temperature=="B",2,
                                 ifelse(spiderr$Temperature=="C",3,
                                        ifelse(spiderr$Temperature=="D",4,
                                               ifelse(spiderr$Temperature=="E",5,
                                                      "NA")))))

spiderr_first<-data.frame("Watershed"=" ", "Site"=1,
                          "E.coli"=1, "Cu_soluble" = 1,"TON"=1,"Turbidity" =1,"Temperature" =1,"Ammonia"=1,
                          "Zn_soluble"=1)
spiderr_second<-data.frame("Watershed"=" ", "Site"=2,
                          "E.coli"=5, "Cu_soluble" = 5 ,"TON"=5,"Turbidity" =5,"Temperature" =5,"Ammonia"=5,
                          "Zn_soluble"=5)
spiderr <- rbind( spiderr_second,spiderr)
spiderr <- rbind( spiderr_first, spiderr)
names_r <- as.character(unique(spiderr$Site[-(1:2)]))
river_spider<-data.frame()
write.csv(spiderr,"spider_ABCD_all_r.csv")
for (i in names_r) {
  # i="Nukumea @ Upper "
  #pull site name, and first two rows to format the data frame (min and max of plot)
  spider_1 <- spiderr[spiderr$Site==i | spiderr$Site==1 | spiderr$Site==2,]
  spider_1 <- spider_1[,-1]
  spider_1 <- sapply( spider_1, as.numeric )
  rownames(spider_1) <- c("1","2",i)

  spider_1 <- spider_1[,-1]
  spider_1 <- as.data.frame(spider_1)
  spider_1 <- spider_1[,c(1:7)] #subset of parameters
  names(spider_1) <- c("E.coli","Copper", "Nitrate","Turbidity", "Temperature", "Ammonia", "Zinc")
  par(bg=NA) #transparent background
  # river_spider<-rbind(river_spider,spider_1)
  if(i == "Nukumea @ Upper "){
    hc_nu<-spider_1
...
write.csv(spiderl,"spider_ABCD_all_l.csv")
names <- as.character(unique(spiderl$Lake_Name[-(1:2)]))
for (i in names) {
  #pull site name, and first two rows to format the data frame (min and max of plot)
  spider_2 <- spiderl[spiderl$Lake_Name==i | spiderl$Lake_Name==1 | spiderl$Lake_Name==2,]
  spider_2 <- spider_2[,-1]
  spider_2 <- sapply( spider_2, as.numeric )
  rownames(spider_2) <- c("1","2",i)
  
  spider_2 <- spider_2[,-1]
  spider_2 <- as.data.frame(spider_2)
  spider_2 <- spider_2[,c(1:5)] #subset of parameters
  names(spider_2) <- c("E.coli", "Phosphorus", "Nitrate", "Ammonia", "Chlorophyll")
  par(bg=NA) #transparent background
  if(i == "Pupuke"){
    hc_pu<-spider_2
  }else if(i == "Tomarata"){
...
ui <- fluidPage(
  ...
  titlePanel("Auckland Current State Tables for Rivers and Lakes"),
    selectInput(
      # Inputs for dropdown selection
    inputId = "wb", label = "Choose:",
      choices = c("Lakes", "Rivers")
    ),
  
  # Main panel for displaying outputs for Rivers----
  mainPanel(conditionalPanel(condition="input.wb == 'Rivers'",
      ...
      tabPanel("Turbidity", DT::dataTableOutput("mytable7")),
      tabPanel("Temperature", DT::dataTableOutput("mytable8"))
    )
  )
 ),
 # Main panel for displaying outputs for Lakes----
 mainPanel(conditionalPanel(condition="input.wb == 'Lakes'",
   tabsetPanel(
     id = 'dataset',
     tabPanel("E.coli", DT::dataTableOutput("mytable9"),
              # WHERE YOUR FOOTER GOES
              hr(),
              print("*No data available for year 2014-2015 for sites - Pupuke, Tomarata, Kuwakatai and Ototoa")),
     tabPanel("Total Phosphorus", DT::dataTableOutput("mytable10")),
     tabPanel("Total Nitrogen", DT::dataTableOutput("mytable11")),
     tabPanel("Total Ammonia", DT::dataTableOutput("mytable12")),
     tabPanel("Chlorophyll", DT::dataTableOutput("mytable13"))
 )
)
)
)
...
server <- function(input, output, session) {
  ...
  # -------------------------------------------------------------------------------
  # To place Turbidity into a html datatable
  output$mytable7<-DT::renderDataTable({DT::datatable(df_tur_r,style="default",
                                                      options=list(columnDefs = list(list(className = 'dt-center', targets=1:8))),
                                                      caption = htmltools::tags$caption(style = 'text-align: left; color:black; font-size: 20px;', 'Table 7: Turbidity'),
                                                      colnames = c('Watershed', 'Site', 'SiteId', 'Sample Count', 'Censored%','5-Year Median', '5-Year Median A.S.', 
                                                                   'Overall Attribute State')
                                                      ,selection = 'none',
                                                      callback = JS("table.on('click.dt', 'td', function() {
                                                                    Shiny.onInputChange('click', Math.random());});")
  ...
  # -----------------------------------------------------------------------------
  # To place Temperature into a html datatable
  output$mytable8<-DT::renderDataTable({DT::datatable(df_tem_r,style="default",
                                                      options=list(columnDefs = list(list(className = 'dt-center', targets=1:10))),
                                                      caption = htmltools::tags$caption(style = 'text-align: left; color:black; font-size: 20px;', 'Table 8: Temperature'),
                                                      colnames = c('Watershed', 'Site', 'SiteId', 'Sample Count', 'Censored%','5-Year Median', "5-Year Max",'5-Year Median A.S.', "5-Year Max A.S.",'Overall Attribute State')
                                                      ,selection = 'none',
                                                      callback = JS("table.on('click.dt', 'td', function() {
                                                                    Shiny.onInputChange('click', Math.random());});")
  ...
  # ---------------------------------------------------------------------------------------------------------------
  # To place Ammonia into html datatable
  output$mytable12<-DT::renderDataTable({DT::datatable(df_amm_l,style="default",
      ...
      formatStyle(2, cursor = 'pointer')})
  # --------------------------------------------------------------------------------------------------------------------------
  # # To place Total Nitrogen into html datatable
  output$mytable11<-DT::renderDataTable({DT::datatable(df_ton_l,style="default",
      ...
      formatStyle(2, cursor = 'pointer')})
  # --------------------------------------------------------------------------------------------------------------------
  # To place Total Phosphorous into a html datatable
  output$mytable10<-DT::renderDataTable({DT::datatable(df_phos_l,style="default",
                                                      options=list(columnDefs = list(list(className = 'dt-center', targets=1:9))),
      ...
      formatStyle(2, cursor = 'pointer')})
  # --------------------------------------------------------------------------------------------------------------------
  # To place Chlorophyll into a html datatable
  output$mytable13<-DT::renderDataTable({DT::datatable(df_chlor_l,style="default",
                                                      options=list(columnDefs = list(list(className = 'dt-center', targets=1:11))),
  ...
  # ----------------------------------------------------------------------------------------------------------------
  # To place E.Coli into a html datatable
  output$mytable9<-DT::renderDataTable({DT::datatable(df_ecoli_l,style="default",
                                                      options=list(columnDefs = list(list(className = 'dt-center', targets=1:14))),
                                                      caption = htmltools::tags$caption(style = 'text-align: left; color:black; font-size: 20px;', 'Table 1: E.Coli'),
  ...
  # -------------------------------------------------------------------------------
  # Spider Plots:
  # To generate the plot outline
  Get_Plot<-function(plot,info){
    output$myplot<-renderPlot({
      radarchart( plot  , axistype=1 , na.itp=FALSE,
  ...
  # To display the plot
  Show_plot<-function(info){
    if (is.null(info$value) || info$col != 2) return()
    if(info$value == "Nukumea @ Upper "){
      plot<-hc_nu
      output$myplot1<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot1")))
    }else if(info$value == "Okura Creek "){
      plot<-hc_oc
      output$myplot2<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot2"))) 
    }else if(info$value == "Vaughn Stream "){
      plot<-hc_vs
      output$myplot3<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot3"))) 
    }else if(info$value == "Waiwera Stream "){
      plot<-hc_ws
      output$myplot4<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot4"))) 
    }else if(info$value == "West Hoe Stream "){
      plot<-hc_wh
      output$myplot5<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot5"))) 
    }else if(info$value == "Cascades @ Whakanewh "){
      plot<-is_cw
      output$myplot6<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot6"))) 
    }else if(info$value == "Onetangi @ Waiheke R "){
      plot<-is_ow
      output$myplot7<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot7"))) 
    }else if(info$value == "Kaukapakapa @Taylors "){
      plot<-ka_kt
      output$myplot8<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot8"))) 
    }else if(info$value == "Kumeu River "){
      plot<-ka_kr
      output$myplot9<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot9"))) 
    }else if(info$value == "Makarau @ Railway "){
      plot<-ka_mr
      output$myplot10<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot10"))) 
    }else if(info$value == "Riverhead Stream "){
      plot<-ka_rs
      output$myplot11<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot11"))) 
    }else if(info$value == "Mahurangi River FHQ "){
      plot<-ma_mf
      output$myplot12<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot12"))) 
    }else if(info$value == "Mahurangi River WS "){
      plot<-ma_mw
      output$myplot13<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot13"))) 
    }else if(info$value == "Ngakaroa Stream "){
      plot<-mh_ns
      output$myplot14<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot14"))) 
    }else if(info$value == "Papakura @ Alfriston "){
      plot<-mh_ps
      output$myplot15<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot15"))) 
    }else if(info$value == "Papakura Stream "){
      plot<-mh_ps
      output$myplot15<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot16"))) 
    }else if(info$value == "Puhinui Stream "){
      plot<-mh_pu
      output$myplot17<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot17"))) 
    }else if(info$value == "Waitangi Falls Br. "){
      plot<-mh_wa
      output$myplot18<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot18"))) 
    }else if(info$value == "Whangamaire Woodhous "){
      plot<-mh_ww
      output$myplot19<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot19"))) 
    }else if(info$value == "Matakana River "){
      plot<-ne_mr
      output$myplot20<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot20"))) 
    }else if(info$value == "Omaru @ Maybury "){
      plot<-ta_om
      output$myplot21<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot21"))) 
    }else if(info$value == "Otaki Creek "){
      plot<-ta_oc
      output$myplot22<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot22"))) 
    }else if(info$value == "Otara Ck East Tamaki "){
      plot<-ta_ot
      output$myplot23<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot23"))) 
    }else if(info$value == "Otara Ck Kennel Hill "){
      plot<-ta_ok
      output$myplot24<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot24"))) 
    }else if(info$value == "Pakuranga Ck Botany "){
      plot<-ta_pc
      output$myplot25<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot25"))) 
    }else if(info$value == "Pakuranga Ck Greenmt "){
      plot<-ta_pg
      output$myplot26<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot26"))) 
    }else if(info$value == "Wairoa @ Caitchons "){
      plot<-wa_wc
      output$myplot27<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot27"))) 
    }else if(info$value == "Wairoa River "){
      plot<-wa_wr
      output$myplot28<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot28"))) 
    }else if(info$value == "Avondale Stream @ Sh "){
      plot<-wt_as
      output$myplot29<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot29"))) 
    }else if(info$value == "Lucas Creek "){
      plot<-wt_lc
      output$myplot30<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot30"))) 
    }else if(info$value == "Oakley Creek "){
      plot<-wt_oc
      output$myplot31<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot31")))
    }else if(info$value == "Opanuku Stream "){
      plot<-wt_os
      output$myplot32<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot32")))
    }else if(info$value == "Oteha Stream "){
      plot<-wt_ot
      output$myplot33<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot33")))
    }else if(info$value == "Parrs Cross "){
      plot<-wt_pc
      output$myplot34<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot34")))
    }else if(info$value == "Rangitopuni River "){
      plot<-wt_rr
      output$myplot35<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot35")))
    }else if(info$value == "Woodside Res "){
      plot<-wt_wr
      output$myplot36<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot36")))
    }else if(info$value == "Cascade Stream "){
      plot<-wc_cs
      output$myplot37<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot37")))
    }
  }
  Show_plot_l<-function(info){
    if (is.null(info$value) || info$col != 2) return()
    if(info$value == "Pupuke"){
      plot<-hc_pu
      output$myplot38<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot38")))
    }else if(info$value == "Tomarata"){
      plot<-ne_to
      output$myplot39<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot39")))
    }else if(info$value == "Kuwakatai"){
      plot<-wc_kw
      output$myplot40<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot40")))
    }else if(info$value == "Ototoa"){
      plot<-wc_oa
      output$myplot41<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot41")))
    }else if(info$value == "Wainamu"){
      plot<-wc_wu
      output$myplot42<-Get_Plot(plot,info)
      removeModal()
      showModal(modalDialog(plotOutput("myplot42")))
    }
  }
  # When the Site is clicked on the html table, repective spider plot is displayed
  ...
  observeEvent(input$mytable12_cell_clicked, {
    info = input$mytable12_cell_clicked
    Show_plot_l(info)
  })
  observeEvent(input$mytable13_cell_clicked, {
    info = input$mytable13_cell_clicked
    Show_plot_l(info)
  })
}
